...
pub fn cp_mv_cln_demo(){
    ...
    let s1 = "XYZ";
    println!("Static String s1 Value: {:?} - Pointer: {:p}", s1, s1.as_ptr());
    let s2 = s1;
    println!("Static String s2 Value: {:?} - Pointer: {:p}", s2, s2.as_ptr());
    println!("Static String s1 Value: {:?} - Pointer: {:p}", s1, s1.as_ptr());
...
String S2 Value: {} - Lenght: {} - FirstByte: {:p}",st2, st2.len(), st2.as_ptr());
    // println!("String S1 Value: {} - Lenght: {} - FirstByte: {:p}",st1, st1.len(), st1.as_ptr());
    // But clonning works
    let st3 = st2.clone();
    println!("String S3 Value: {} - Lenght: {} - FirstByte: {:p}",st3, st3.len(), st3.as_ptr());
    println!("String S2 after cloning Value: {} - Lenght: {} - FirstByte: {:p}",st2, st2.len(), st2.as_ptr());
